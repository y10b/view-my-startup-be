...
오늘도 번개장터는 더 쉽고 안전한 개인간 거래를 만들어갑니다`,
  ...
  const rows = companies.map((c) => ({
    ...
    category: c.category,
    totalInvestment: Math.round(Number(c.realInvestmentAmount ?? 0)),
    revenue: Math.round(Number(c.revenue ?? 0)),
    employees: Math.round(Number(c.employeesNumber ?? 0)),
    selectedCompany: Math.round(Number(c.selectedNumber ?? 0)),
    comparedCompany: 0,
  ...
  await prisma.$transaction(
    rows.map((row) =>
      prisma.$executeRaw`
        INSERT INTO "Company" ("name","description","category","totalInvestment","revenue","employees","createdAt","updatedAt","imageUrl","selectedCompany","comparedCompany")
        VALUES (${row.name}, ${row.description}, ${row.category}, ${row.totalInvestment}, ${row.revenue}, ${row.employees}, ${row.createdAt}, ${row.updatedAt}, ${row.imageUrl}, ${row.selectedCompany}, ${row.comparedCompany})
        ON CONFLICT ("name") DO UPDATE SET 
          ...
          "selectedCompany" = EXCLUDED."selectedCompany",
          "comparedCompany" = EXCLUDED."comparedCompany";
      `
    )
  );
}
